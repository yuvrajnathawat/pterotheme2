...
class WebsocketController extends ClientApiController
{
    ...
    public function __invoke(ClientApiRequest $request, Server $server): JsonResponse
    {
        ...
        // Always use the encrypted WebSocket scheme (wss://) when the panel itself is served
        // over HTTPS. Browsers block mixed-content ws:// connections from https:// pages.
        $address = $node->getConnectionAddress();
        $socket = str_replace(['https://', 'http://'], ['wss://', 'ws://'], $address);
        if ($request->isSecure() && str_starts_with($socket, 'ws://')) {
            $socket = 'wss://' . substr($socket, strlen('ws://'));
        }
