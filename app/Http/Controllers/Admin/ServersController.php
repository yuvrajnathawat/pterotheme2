...
class ServersController extends Controller
{
    ...
    public function setDetails(Request $request, Server $server): RedirectResponse
    {
        $this->detailsModificationService->handle($server, $request->only([
            'owner_id', 'external_id', 'name', 'description', 'exp_date', 'product_id',
        ]));
    ...
    public function updateBuild(Request $request, Server $server): RedirectResponse
    {
        try {
            $this->buildModificationService->handle($server, $request->only([
                ...
                'server_type_changer_allowed',
            ]));
        } catch (DataValidationException $exception) {
            throw new ValidationException($exception->getValidator());
        }

        // Update Server Splitter whitelist limit
        if ($request->has('split_limit')) {
            ...
            if ($splitLimit > 0) {
                ...
                ServerSplitterWhitelist::where('server_id', $server->id)->delete();
            }
        }

        // Update Reverse Proxy whitelist limit
        if ($request->has('proxy_limit')) {
            ...
            if ($proxyLimit > 0) {
                ...
                ReverseProxyWhitelist::where('server_id', $server->id)->delete();
            }
        }
