    ...
    <div class="col-sm-12">
        <div class="box box-primary">
            <div class="box-header with-border">
                ...
                <div class="box-tools">
                    <form action="{{ route('admin.nodes.view.servers', $node->id) }}" method="GET" class="form-inline" style="display: inline-block;">
                ...
                <table class="table table-hover">
                    <tbody>
                        <tr>
                            ...
                            @endif
                            <th><a href="{{ route('admin.nodes.view.servers', $node->id) }}?{{ http_build_query(array_merge(request()->query(), ['sort' => request('sort') === 'name' ? '-name' : 'name'])) }}">Server Name</a></th>
                            <th>UUID</th>
                            <th><a href="{{ route('admin.nodes.view.servers', $node->id) }}?{{ http_build_query(array_merge(request()->query(), ['sort' => request('sort') === 'owner' ? '-owner' : 'owner'])) }}">Owner</a></th>
                            <th>Connection</th>
                            <th>Type</th>
                            <th><a href="{{ route('admin.nodes.view.servers', $node->id) }}?{{ http_build_query(array_merge(request()->query(), ['sort' => request('sort') === 'status' ? '-status' : 'status'])) }}">Status</a></th>
                            <th><a href="{{ route('admin.nodes.view.servers', $node->id) }}?{{ http_build_query(array_merge(request()->query(), ['sort' => request('sort') === 'exp_date' ? '-exp_date' : 'exp_date'])) }}">Expiry</a></th>
                        </tr>
